...
async def start(_, message):
    ...
    buttons = ButtonMaker()
    buttons.url_button(
        lang.START_BUTTON1, "https://www.github.com/Hemanth/HemanthBot"
    )
    buttons.url_button(lang.START_BUTTON2, "http://t.me/ALONEKINGSTAR77")
    reply_markup = buttons.build_menu(2)

    if len(message.command) > 1 and message.command[1] == "HemanthBot":
        ...
        if Config.MEDIA_STORE and decrypted_url.startswith("file"):
            ...
            buttons.data_button(
                "𝓐𝓬𝓽𝓲𝓿𝓪𝓽𝓮 𝓐𝓬𝓬𝓮𝓼𝓼 𝓣𝓸𝓴𝓮𝓷", f"start pass {input_token}", "header"
            )
            reply_markup = buttons.build_menu(2)
            msg = f"""⌬ 𝓐𝓬𝓬𝓮𝓼𝓼 𝓛𝓸𝓰𝓲𝓷 𝓣𝓸𝓴𝓮𝓷 :
    │
    ┟ <b>𝓢𝓽𝓪𝓽𝓾𝓼</b> → <code>Generated Successfully</code>
    ┟ <b>𝓐𝓬𝓬𝓮𝓼𝓼 𝓣𝓸𝓴𝓮𝓷</b> → <code>{input_token}</code>
    ┃
    ┖ <b>𝓥𝓪𝓵𝓲𝓭𝓲𝓽𝔂:</b> {get_readable_time(int(Config.VERIFY_TIMEOUT))}"""
