...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    msg += "⌬ <b><u>𝐁𝐨𝐭 𝐒𝐭𝐚𝐭𝐬</u></b>"
    buttons = ButtonMaker()
    if not is_user:
        buttons.data_button("📜 𝓣𝓢𝓽𝓪𝓽𝓼", f"status {sid} ov", position="header")
    if len(tasks) > STATUS_LIMIT:
        msg += f"<b>𝓟𝓪𝓰𝓮:</b> {page_no}/{pages} | <b>𝓣𝓪𝓼𝓴𝓼:</b> {tasks_no} | <b>𝓢𝓽𝓮𝓹:</b> {page_step}\n"
    ...
    buttons.data_button("♻️ 𝓡𝓮𝓯𝓻𝓮𝓼𝓱", f"status {sid} ref", position="header")
    button = buttons.build_menu(8)
    msg += f"\n┟ <b>𝘾𝙋𝙐</b> → {cpu_percent()}% | <b>𝙁</b> → {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)} [{round(100 - disk_usage(DOWNLOAD_DIR).percent, 1)}%]"
    msg += f"\n┖ <b>𝙍𝘼𝙈</b> → {virtual_memory().percent}% | <b>𝙐𝙋</b> → {get_readable_time(time() - bot_start_time)}"
    return msg, button
